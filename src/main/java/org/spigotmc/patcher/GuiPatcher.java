...
public class GuiPatcher extends JFrame {
	...
	//Downloadable File's key list: (DownloadableItems.keySet() is too random)
	private static List<String> sortedNames = new ArrayList<String>();
	//Files:
	private static File original = null, bps = null, result = null;
	//Version Strings:
	private static String newest_version = "N/A", version = "1.0.7";
	//The instance:
	private static GuiPatcher instance;
			...
			try {
				...
				while((output = bufferedreader.readLine()) != null) {
					if(output.split("[;]").length == 2) {
						...
						sortedNames.add(output.split("[;]")[0]);
						System.out.println("> "+output.split("[;]")[0]+" -> "+output.split("[;]")[1]);
					} else {
				...
				if((output = bufferedreader.readLine()) != null) {
					...
					if(!newest_version.equals("v"+version))
	...
	private GuiPatcher() {
		...
		setTitle("Spigot Patcher v"+version);
		setType(Type.POPUP);
		setResizable(false);
		try {
		...
		NewestVersionLable.setForeground(Color.WHITE);
		...
		DownloadList.setModel(new AbstractListModel<String>() {
			...
			String[] values = (String[]) sortedNames.toArray(new String[sortedNames.size()]);
			public int getSize() {
				...
				try {
					...
					if (fc.showSaveDialog(GuiPatcher.this) == JFileChooser.APPROVE_OPTION) {
						out = new ByteArrayOutputStream();
				   		...
				   		JOptionPane.showMessageDialog(null, LanguageHelper.getLang().INFO_DOWNLOAD.replace("<file>", fc.getSelectedFile().getName()).replace("<size>", ""+total), "Spigot Patcher", JOptionPane.INFORMATION_MESSAGE);
					}
				} catch(Exception e) {
	 ...
	 * @return
	 */
	public static GuiPatcher getInstance() {
		return instance;
	}
